USE [odbphy]
GO
/****** Object:  StoredProcedure [dbo].[sadcpgridqry]    Script Date: 2026/9/10 上午 10:48:50 ******/
SET ANSI_NULLS ON
GO
...
IF (@nmorng <> N'')
  BEGIN
  IF (@mode = 'raw')
    SET @sqlcmd = 'SELECT longitude,latitude,time_period,depth,' + @append + ',year,month from (' + @sqlcmd + ') a where a.' + @nmorng;
  ELSE
    SET @sqlcmd = 'SELECT longitude,latitude,time_period,depth,' + @append + ' from (' + @sqlcmd + ') a where a.' + @nmorng;
  END
ELSE
  BEGIN
  IF (@mode = 'raw')
    SET @sqlcmd = 'SELECT longitude,latitude,time_period,depth,' + @append + ',year,month from (' + @sqlcmd + ') a';
  ELSE
